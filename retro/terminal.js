...
class Texel {
   ...
   * @param {string} fg The foreground color (expressed as '#xxxxxx').
   * @param {string} bg The background color (expressed as '#xxxxxx').
   */
...
class CanvasTerm {
  constructor(cols=80, rows=25) {
    ...
    /** @type {Promise} Set by input() and resolved when the users presses Enter. */
  ...
  clear() {
    ...
    document.getElementById('basic').style.backgroundColor = this.colorBg.hex;
  }
...
module.exports = {
   ...
   */
  async input(prompt = '') {
    return await term.input(`{_}${prompt}{Y}`);
  },
